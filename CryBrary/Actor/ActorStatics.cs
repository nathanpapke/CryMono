...
namespace CryEngine
{
    public abstract partial class Actor
    {
        ...
        public static ActorBase Get(int channelId)
        {
            ...
            var entityInfo = NativeActorMethods.Instance.GetActorInfoByChannelId((ushort)channelId);
            if (entityInfo.Id != 0)
        ...
        public static ActorBase Get(EntityId actorId)
        {
            ...
            var actorInfo = NativeActorMethods.Instance.GetActorInfoById(actorId);
            if (actorInfo.Id != 0)
        ...
        public static ActorBase LocalClient
        {
            get
            {
                var clientActorId = NativeActorMethods.Instance.GetClientActorId();
                if (clientActorId == 0)
                    return null;
        ...
        internal static T CreateCommon<T>(string className, int channelId, string name = "Dude", Vec3? pos = null, Quat? rot = null, Vec3? scale = null) where T : ActorBase
        {
            ...
            var info = NativeActorMethods.Instance.CreateActor(channelId, name, className, pos ?? new Vec3(0, 0, 0), rot ?? Quat.Identity, scale ?? new Vec3(1, 1, 1));
            if (info.Id == 0)
        ...
        {
            NativeActorMethods.Instance.RemoveActor(id);
        }
        ...
        {
            var actorInfo = NativeActorMethods.Instance.GetActorInfoByChannelId((ushort)channelId);
            if (actorInfo.Id != 0)
                NativeActorMethods.Instance.RemoveActor(actorInfo.Id);
        }
    }
}
